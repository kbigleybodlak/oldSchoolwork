...
void* produce(LPVOID lpParameter);
void* consume(LPVOID lpParameter);
...
void main(int argc, char* argv[]){
	...
	int num_to_produce;
	int num_to_consume;
	pthread_t last_producer_thread;
	pthread_t last_consumer_thread;
	...
	for(i = 0; i<num_producers;i++){
		...
		pthread_create(&last_producer_thread,NULL,&produce,&producer_info[i]);
	}
	...
	for(i = 0; i<num_consumers;i++){
		...
		pthread_create(&last_consumer_thread,NULL,&consume,&consumer_info[i]);
	}
	//wait for thread resolutions. waitng for only the last thread created works fine.
	pthread_join(last_producer_thread,NULL);
	pthread_join(last_consumer_thread,NULL);
	free(&producer_info);
	free(&consumer_info);
	sem_destroy(&full);
	sem_destroy(&empty);
